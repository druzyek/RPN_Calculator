...
static void TanBCD(unsigned char *sine_result,unsigned char *cos_result,unsigned char *arg)
{
  ...
  gotoxy(40,0);
  printf("%x%x",((unsigned int)sine_result)>>8,((unsigned int)sine_result)&0xFF);
  putchar('!');
  gotoxy(40,1);
  printf("%x%x",((unsigned int)cos_result)>>8,((unsigned int)cos_result)&0xFF);
  putchar('!');
  gotoxy(40,2);
  printf("%x%x",((unsigned int)arg)>>8,((unsigned int)arg)&0xFF);
  putchar('!');
...
static void AtanBCD(unsigned char *result,unsigned char *arg)
{
  ...
  //result[BCD_LEN]=Settings.DecPlaces-result[BCD_DEC];
...
int main()
{
  ...
  ClrLCD();
  gotoxy(-1,6);
  const char legend[36][10]={"atan","!prog","cos","dupe","e^x","","acos","asin","pi","10^x","log","ln","+/-","1/x","round","y^x","sqrt","x root y","sin","tan","!SETTINGS",
